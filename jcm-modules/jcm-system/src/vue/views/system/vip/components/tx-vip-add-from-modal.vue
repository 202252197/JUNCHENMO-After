<template>
  <el-dialog v-model="fromOpenStatus" width="500" :show-close="false">
    <template #header="{ titleId, titleClass }">
      <div class="my-header">
        <h4 :id="titleId" :class="titleClass">新增vip</h4>
      </div>
    </template>
    <el-form :model="txVipStore.commonForm" label-width="80" :rules="formRules" ref="formRef">
                      <el-form-item label="咸鱼名称" prop="xyName">
                        <el-input v-model="txVipStore.commonForm.xyName" placeholder="请输入咸鱼名称" />
                      </el-form-item>
...
<script setup lang="ts">
  ...
  const addItem =(formEl: FormInstance | undefined) => {
    ...
    formEl.validate((valid) => {
      if (valid) {
          txVipStore.add(txVipStore.commonForm).then(() => {
